...
import fr.isika.al17.raiddonspringserver.payload.request.SignupRequest;
import fr.isika.al17.raiddonspringserver.payload.response.MessageResponse;
import fr.isika.al17.raiddonspringserver.payload.response.UserInfoResponse;
import fr.isika.al17.raiddonspringserver.repository.RoleRepository;
...
public class AuthController {
	...
	if (userRepository.existsByUsername(signUpRequest.getUsername())) {
	    return ResponseEntity.badRequest().body(new MessageResponse(Constants.USERNAME_EXISTS));
	}

	if (userRepository.existsByEmail(signUpRequest.getEmail())) {
	    return ResponseEntity.badRequest().body(new MessageResponse(Constants.EMAIL_EXISTS));
	}
